...
class SRnet(object):
    def __init__(self, phase='train'):
        ...
        self.datasets = tf.data.TFRecordDataset(self.files)
        #self.datasets.map(lambda x: tf.image.resize_bicubic(x, (x.shape[1]*2, x.shape[2]*2))        
        self.datasets = self.datasets.map(self.parser).repeat().batch(128).shuffle(buffer_size=1000)
        
        self.iterator = self.datasets.make_one_shot_iterator()
    ...
    def parser(self, record):
        self.features = tf.parse_single_example(record, features={
        'label': tf.FixedLenFeature([], tf.string), 
        'img_raw': tf.FixedLenFeature([], tf.string)}
        )        
        
        img = tf.decode_raw(self.features['img_raw'], tf.uint8)
        label = tf.decode_raw(self.features['label'], tf.uint8)
